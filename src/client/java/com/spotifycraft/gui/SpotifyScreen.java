...
public class SpotifyScreen extends Screen {
    ...
    private void renderProfile(DrawContext ctx) {
        ...
        // Device status display - DETAILED DEBUG INFO
        int deviceY = avatarY + avatarSize + 50;

        // Show auth status
        String authStatus = SpotifyApi.isAuthenticated() ? "✓ Authenticated" : "✗ Not Authenticated";
        int authColor = SpotifyApi.isAuthenticated() ? 0xFF1DB954 : 0xFFCC0000;
        ctx.drawTextWithShadow(textRenderer, Text.literal(authStatus),
                px + PAD, deviceY, authColor);

        // Show current track
        String trackStatus = SpotifyApi.currentTrack != null ?
                "Track: " + SpotifyApi.currentTrack.title() : "No track detected";
        ctx.drawTextWithShadow(textRenderer, Text.literal(trackStatus),
                px + PAD, deviceY + 12, 0xFFFFFF);

        // Show playing status
        String playingStatus = "Playing: " + (SpotifyApi.isPlaying ? "Yes" : "No");
        ctx.drawTextWithShadow(textRenderer, Text.literal(playingStatus),
                px + PAD, deviceY + 24, 0xFFFFFF);

        // Show volume
        ctx.drawTextWithShadow(textRenderer, Text.literal("Volume: " + (int)SpotifyApi.volume + "%"),
                px + PAD, deviceY + 36, 0xFFFFFF);

        // Show error message (the important part)
        if (SpotifyApi.lastError != null && !SpotifyApi.lastError.isBlank()) {
            String error = SpotifyApi.lastError;
            if (error.length() > 40) error = error.substring(0, 40) + "...";
            ctx.drawTextWithShadow(textRenderer, Text.literal("Error: " + error),
                    px + PAD, deviceY + 48, 0xFFCC6666);
        }
    }
